...
async function ensureConversation(user) {
  ...
  const { data: newConv } = await supabase
    ...
    .insert({
      partner_id: user.id,
      partner_name: user.user_metadata?.full_name || user.user_metadata?.name || user.email?.split('@')[0] || 'Partner',
...
function renderConversationList(conversations, isTeam) {
  ...
  if (conversations.length === 0) {
    clist.innerHTML = `<div style="padding:32px 22px;text-align:center;color:var(--muted);font-size:12px">${isTeam ? 'No partner messages yet' : 'Tap New Message to start chatting'}</div>`;
    return;
  }
  clist.innerHTML = conversations.map(conv => {
    const name = isTeam ? (conv.partner_name || 'Partner') : 'Angora Team';
    const initials = isTeam ? getInitials(conv.partner_name) : 'AT';
    const preview = conv.last_message ? escapeHtml(conv.last_message.slice(0, 42)) + (conv.last_message.length > 42 ? '…' : '') : 'Start a conversation';
    const time = conv.last_message_at ? msgFormatTime(conv.last_message_at) : '';
    const unread = isTeam && conv.has_unread;
    const gradient = isTeam ? 'linear-gradient(135deg,#6b4fcc,#0A0A0A)' : 'linear-gradient(135deg,#6b4fcc,#0A0A0A)';
    return `<div class="crow ${unread ? 'unread' : ''}" data-conv-id="${conv.id}" data-partner-name="${escapeHtml(conv.partner_name || 'Partner')}">
      <div class="cav" style="background:${gradient}">${initials}${unread ? '<div class="conl"></div>' : ''}</div>
  ...
  clist.querySelectorAll('.crow').forEach(row => {
    row.addEventListener('click', () => {
      ...
      openConversationById(convId, partnerName, isTeam);
    });
  });
...
function renderMessageList(messages, isTeam) {
  ...
  if (messages.length === 0) {
    list.innerHTML = `<div style="flex:1;display:flex;align-items:center;justify-content:center;padding:40px 0;color:var(--muted);font-size:12px;text-align:center">Send a message to start the conversation</div>`;
    return;
  }
...
async function sendChatMessage() {
  ...
  await supabase.from('conversations').update({
    ...
    has_unread: !isTeam, // mark unread for team when partner sends
